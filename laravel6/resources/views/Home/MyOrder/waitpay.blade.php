...
<div class="main-wrap mt15" style="border: 0px;">
    <ul class="sui-nav nav-tabs" style="margin-top:0px;width: 1000px;margin-left: 30px;">
        <li style="margin-left: -5px;" class=""><a href="#profile" data-toggle="tab"><a href="myorder">所有订单</a><span style="margin-left: 20px;color: #ccc;">|</span></a></li>
  ...
  <div class="tab-content" style="width: 1000px;margin-top: 10px;border:1px #fff solid; border-top:transparent;margin-left: 30px;">
      ...
      <span style="line-height: 50px;margin-left: 20px;">{{$v->order_num}}</span>
       ...
       <div style="float: left;border-left: 1px #ccc solid;width: 11%;height:100px;text-align: center ;">
       <button style="color: fff;background-color: #65b5ff;border: 0px;padding: 4px;margin-top: 38px;">@if($v->status==0)<a href="/yespay/{{$v->id}}">去付款</a></button>@elseif($v->status==1)<a name="{{$v->id}}" class="ajaxtixing">提醒发货</a></button>@elseif($v->status==2)<a href="/yesreceipt/{{$v->id}}">确认收货</a>@elseif($v->status==3)<a href="">去评价</a>@endif
       </div>
       @else
...
<script>
$(".ajaxtixing").click(function(){
    //alert($);
    id=$(this).attr("name");
    o=$(this);
    //alert(id);
    $.get("/tixingreceipt",{id:id},function(data){
        //alert(data);
        if(data){
            o.html("已通知发货");
        }
