...
$(function(){
	try{
		if($(".left-side").length>0 && domain.my == window.location.protocol+"//"+window.location.host){
			$.ajax({
				url : domain.my + "/ym/isBindFirstMedicine.html",
				type : "POST",
				success : function(data) {
					if(null!=data&&data=="1"){
						$("#idDyyy").html("<dd><a href=\""+domain.my+"/dyyy/orderList.html\">我的医药订单</a></dd>");
					}
				}
			});
		}
		//去除游戏中心
//		$(".pay-head").each(function(){
//			if($(this).html().indexOf("游戏中心")>=0){
...
//		});
	...
	$("#user_not_login").find(".login-button").click(function(event) {
		if($("#rside").length > 0){
            ...
            var hrefReturn = domain.passport+ "?returnurl="+ encodeURIComponent(window.location.href);
            if(addUrl){
            	hrefReturn += hrefReturn+addUrl;
            }
			$(this).attr("href",hrefReturn);
		}
    });
...
//	
	...
	$.ajax({
		...
		success : function(data){
			if(data != null){
				var toolpalm = "";
				toolpalm = "<div class='tools-leftfont'><a href='javascript:;'><span>掌上百联</span><i></i></a></div>"
    				+ "<span class='left-span'></span>"
    				+ "<b></b>"
    				+ "<div class='divshow palm'>";
				//微信公众号二维码
				for(var i = 0 ; i < data.obj.length ; i++){
    				if('2' == data.obj[i].priority){
    					toolpalm += "<div class='palm-item'>"
    						+ "<span class='palm-item-img'><img src='"+data.obj[i].mediaUrl+"' width='100' height='100'></span>"
    						+ "<div class='palm-item-line'><span class='palm-item-title'>"+data.obj[i].picDesc1+"</span></div>"		
    						+ "<div class='palm-item-line'><span class='palm-item-message'>"+data.obj[i].picDesc2+"</span></div>"		
    						+ "</div>";
    					break;
    				}
				}
